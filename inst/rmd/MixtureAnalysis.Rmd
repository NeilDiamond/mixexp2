...
```{r}
mixexp::DesignPoints(0.2+0.4*des, x1lower=0.2,x1upper=0.6,x2lower=0.2,x2upper=0.6,x3lower=0.2,x3upper=0.6,
             pseudo=T)
```
...
mixexp::MixturePlot(des=desrun1, mod=2)
mixexp::MixturePlot(des=desrun1,  mod=2, cols=T)
```
...
MixtureAnalysis <- function(DF, resp="y"){
  ...
  fit <- eval(parse(text=paste0("rsm(respval ~ FO(",xvars1,")+TWI(", xvars2,"),data=DF)")))
...
```{r}
soapdes <- mixexp::Xvert(4, uc=c(0.4,0.4,0.7,0.4), lc=c(0.05,0.05,0.30,0.05))
...
mixexp::DesignPoints(soapdeswithaxial[near(soapdeswithaxial$x1,min(soapdeswithaxial$x1)),-1], 
...
mixexp::DesignPoints(soapdeswithaxial[near(soapdeswithaxial$x2,min(soapdeswithaxial$x2)),-2])
mixexp::DesignPoints(soapdeswithaxial[near(soapdeswithaxial$x3,min(soapdeswithaxial$x3)),-3])
mixexp::DesignPoints(soapdeswithaxial[near(soapdeswithaxial$x4,min(soapdeswithaxial$x4)),-4])
